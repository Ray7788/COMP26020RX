...
-- Exercise 2
-- output the size of a QuadTree by calculating the length of one side (top)
...
-- detect one specified side of a certain Gird/Cell and sum its values depends on colors
...
-- (initial QuadTree, initial QuadTree size) -> Cell's 
...
-- for Grid
blurMini (Grid a b c d) (fullTree, treeSize) (x_now, y_now, sizeNow) =
  clockwise
    (blurMini a (fullTree, treeSize) (x_now + offset, y_now - offset, sizeNow / 2))
    (blurMini b (fullTree, treeSize) (x_now - offset, y_now - offset, sizeNow / 2))
    (blurMini c (fullTree, treeSize) (x_now - offset, y_now + offset, sizeNow / 2))
    (blurMini d (fullTree, treeSize) (x_now + offset, y_now + offset, sizeNow / 2))
  where
    offset = sizeNow / 4
...
blur fullTree = let treeSize = (getTreeSize fullTree)
  in (blurMini fullTree (fullTree, treeSize) (treeSize / 2, treeSize / 2, treeSize))
